...
(function () {
  ...
  const ELEMENTS = [
    ...
    { id: 'block_fall', cat: 'struct', name: '坠落砖组', kind: 'vector', img: 'block/block_brick.png',
      ...
      hint: 'stype=51：连在一起的砖块（默认3块横排）。玩家完全进入砖组长轴区域并处于运动方向一侧时，砖组加速坠落/平移；移动中碰到玩家即阵亡。可配置延时(秒，默认0=立即坠落)。选中后在⚙属性里改排列(横/竖)、砖块数(2-12)、移动方向(上/下/左/右)、延时' },
    { id: 'block_fall_d', cat: 'struct', name: '坠落砖组·延时', kind: 'vector', img: 'block/block_d_brick.png',
      ...
      hint: 'stype=51/sxtype=1：地下砖样式的坠落砖组（1-2-1 连锁崩塌桥）。触发后延迟指定秒数再坠落（默认0.5秒，延时期间可踩）。选中后在⚙属性里改排列、砖块数(2-12)、移动方向、延时(秒)' },
